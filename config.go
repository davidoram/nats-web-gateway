...
// DownstreamIdentity declares a NATS-authenticated attribute to add to service
// requests. The configured header is generated by the gateway and can never be
// populated from the HTTP request.
...
func (security SecurityContext) validate() error {
	...
	if security.DownstreamIdentity != nil {
		if err := security.DownstreamIdentity.validate(); err != nil {
			return fmt.Errorf("downstream_identity: %w", err)
		}
	}
	return nil
}

func (identity DownstreamIdentity) validate() error {
	if identity.Source != downstreamIdentitySourceNATSUserID {
		return fmt.Errorf("unsupported source %q", identity.Source)
	}
	if !httpguts.ValidHeaderFieldName(identity.Header) || http.CanonicalHeaderKey(identity.Header) != identity.Header {
		return fmt.Errorf("header must be a canonical HTTP header name, got %q", identity.Header)
	}
	if forbiddenGeneratedIdentityHeader(identity.Header) {
		return fmt.Errorf("header %q is reserved by HTTP or NATS", identity.Header)
	}
	if identity.MaxValueBytes <= 0 {
		return errors.New("max_value_bytes must be greater than zero")
	}
	return nil
}
...
func forbiddenGeneratedIdentityHeader(header string) bool {
	switch header {
...
func unmarshalRoute(d *caddyfile.Dispenser) (Route, error) {
	...
	for d.NextBlock(0) {
		...
		case "downstream_identity_source":
			if !d.AllArgs(&ensureDownstreamIdentity(&route).Source) {
				return route, d.ArgErr()
			}
		case "downstream_identity_header":
			if !d.AllArgs(&ensureDownstreamIdentity(&route).Header) {
				return route, d.ArgErr()
			}
		case "max_downstream_identity_bytes":
			if err := parsePositiveInt(d, &ensureDownstreamIdentity(&route).MaxValueBytes); err != nil {
				return route, err
			}
		case "response_mode":
...
func ensureDownstreamIdentity(route *Route) *DownstreamIdentity {
	security := ensureSecurityContext(route)
	if security.DownstreamIdentity == nil {
		security.DownstreamIdentity = new(DownstreamIdentity)
	}
	return security.DownstreamIdentity
}
